...
class ApiRoutes {
  ...
  _registerAuthRoutes() {
    ...
    this.app.post('/api/', (req, res) => {
      if (!process.env.ZOOM_VSDK_KEY || !process.env.ZOOM_VSDK_SECRET) {
        return res.status(503).json({ error: 'Zoom Video SDK credentials are not configured. Set ZOOM_VSDK_KEY and ZOOM_VSDK_SECRET in your .env file.' })
      }
      ...
      const checks = [
        {
          ...
          ok: !!(process.env.ZOOM_VSDK_KEY && process.env.ZOOM_VSDK_SECRET),
          detail: !process.env.ZOOM_VSDK_KEY ? 'ZOOM_VSDK_KEY missing' :
                  !process.env.ZOOM_VSDK_SECRET ? 'ZOOM_VSDK_SECRET missing' : null,
          required: true
        },
        {
          key: 'openai',
          label: 'OpenAI (AI responses)',
          ok: !!(process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY.trim()),
          detail: !process.env.OPENAI_API_KEY ? 'OPENAI_API_KEY missing — using fallback responses' : null,
          required: false
        },
        {
          key: 'aws_polly',
          ...
          detail: !process.env.AWS_ACCESS_KEY_ID ? 'AWS credentials missing — using browser TTS' : null,
          required: false
        },
        {
          key: 'speechgen',
          label: 'SpeechGen (TTS)',
          ok: !!(process.env.SPEECHGEN_API_TOKEN && process.env.SPEECHGEN_EMAIL),
          detail: !process.env.SPEECHGEN_API_TOKEN ? 'SPEECHGEN_API_TOKEN missing — using browser TTS' : null,
          required: false
        }
      ]
  ...
  _registerSettingsRoutes() {
    ...
    this.app.post('/api/settings', async (req, res) => {
      try {
        // Experiment conditions are owned exclusively by the /api/conditions
        // endpoints. A full-settings save must never carry conditions from the
        // client body: the admin UI holds a conditions snapshot taken at page
        // load, so saving unrelated settings (silence/periodic/LLM) would write
        // back that stale snapshot and silently wipe conditions created since.
        // Always preserve the server's current conditions here.
        const current = await this.settingsManager.load()
        const { conditions: _ignoredConditions, ...incoming } = req.body || {}
        const savedSettings = await this.settingsManager.save({
          ...current,
          ...incoming,
    ...
    // GET /api/sessions/:id/transcript.csv - Server-rendered transcript CSV
    // for the current room. Uses the same field layout as the on-disk
    // transcript.csv written when the session closes.
    this.app.get('/api/sessions/:id/transcript.csv', (req, res) => {
      try {
        const { id } = req.params
        const room = this.sessionManager.getSessionRoom
          ? this.sessionManager.getSessionRoom(id)
          : this.sessionManager.sessionRooms.get(id)

        if (!room) {
          return res.status(404).json({ error: `Session "${id}" not found or already ended` })
        }

        const csv = this.logger.buildTranscriptCsv(room.conversationHistory || [])
        res.setHeader('Content-Type', 'text/csv; charset=utf-8')
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="transcript-${id.replace(/[^a-zA-Z0-9-_]/g, '_')}.csv"`
        )
        res.send(csv)
      } catch (err) {
        this.logger.logError('SESSION-TRANSCRIPT-API', 'Failed to build transcript CSV', {
          error: err.message
        })
        res.status(500).json({ error: 'Failed to build transcript CSV', message: err.message })
      }
    })
  }
}
