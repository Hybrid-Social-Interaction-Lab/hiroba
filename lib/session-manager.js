...
class SessionManager {
  ...
  addClientToSession(ws, sessionId, userInfo) {
    ...
    // Seed metadata that's stable across the session.
    this.logger.setSessionMetadata(sessionId, {
      topic: room.topic,
      conditionId: room.conditionId,
      startTime: room.createdAt instanceof Date ? room.createdAt.toISOString() : room.createdAt
    })
        ...
        if (session.isHost) {
          ...
          if (remainingRegularClients.length > 0) {
            ...
            if (newHostSession) {
              ...
              this.logger.setSessionMetadata(session.sessionId, {
                hostUserId: newHostSession.userId,
                hostUserName: newHostSession.userName
              })
